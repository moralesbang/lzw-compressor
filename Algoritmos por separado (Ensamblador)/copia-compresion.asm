...
main:
	...
	jal compression
	j exit
#---------------------Metodo de Compresiï¿½n (dicc.dir, file.dir)-----------------------------------------------
compression: 
	...
	addi	$t0, $t0, 1			# i = i+1
	whileCNull:
		beq	$t1, $zero, ExitWhileCNull  ##------------------------------- $zero es la representaciï¿½n de null?-------		
		...
		jal 	lenghtDic
		addi	$a2, $v0, 0		# parametro de tamaï¿½o del diccionario
		...
		###-------------------
		li	$v0, 1
		addi 	$a0, $t4,0		# posición
		syscall
		
		
		li	$v0, 4
		la $a0, separator		# direcciÃ³n de la cadena
		syscall
		#----------------------------------------
...
storeInDic:
	...
	jal lenght
	addi	$t0, $v0, 0			#Tamaï¿½o del vector aux
	addi 	$t1, $s0, 0			#i = 0
...
concatenate:
	...
	addi 	$s0, $a0, 0			#Direcciï¿½n del vector C
	jal	lenght
	addi	$s1, $v0, 0			#tamaï¿½o del vector C
